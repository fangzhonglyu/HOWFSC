...
def plot_perf(compute_specs: list[ComputeSpec], kernels: list[Kernel]=None):
    ...
    for kernel in kernels or []:
        ...
        oi = kernel.FLOPs / kernel.mem_access
        print(f'Kernel {kernel.name} OI: {oi:.2f} FLOP/Byte')
        nbytes = np.logspace(0, 6, 200)
        plt.plot(nbytes * oi, nbytes, '--')
        # Slanted Line
        # plt.plot([1, 1e6*oi], [1, 1e6], '--', label=f'{kernel.name} (OI={oi:.2f})')

        plt.fill_between([0,kernel.FLOPs], 0, [0,kernel.mem_access], color='lightsteelblue', alpha=0.5)
        plt.text(1e4, 3, 'Memory Bound', rotation=0, color='steelblue', fontsize=16)
        plt.fill_between([0, kernel.FLOPs], [0,kernel.mem_access], 1e7, color='moccasin', alpha=0.5)
        plt.text(3, 4e4, 'Compute Bound', rotation=0, color='darkorange', fontsize=16)

        for iter_per_sec in [1, 10, 100]:
            # Plot kernel required architecture agnostic performance points
            x = kernel.FLOPs * iter_per_sec / 1e9
            y = kernel.mem_access * iter_per_sec / 1e9
            print(f'Kernel {kernel.name} Iter/sec {iter_per_sec}: Requires {x:.2f} GFLOPs, {y:.2f} GB/s')
            plt.scatter(x, y, label=f'{kernel.name} (Iter/sec={iter_per_sec})', color=['black'])

            # Instead of scatter point, plot vertical and horizontal lines, ending at the point
            plt.plot([x, x], [y, 1e9], color='gray', linestyle='--', linewidth=0.8)
            plt.plot([x, 1e9], [y, y], color='gray', linestyle='--', linewidth=0.8)

            plt.annotate(f'{iter_per_sec} Hz',
                         (x, y),
                         textcoords="offset points", xytext=(5,50), ha='left', fontsize=12)
...
if __name__ == '__main__':
    ...
    plot_perf(compute_specs, kernels=kernels)
